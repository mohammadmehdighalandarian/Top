...
namespace TopinLite.CrmTransform.IntegrationEnquery
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            app.MapPost("/IntegrationEnquery", async (HttpContext httpContext, [FromServices] IMediator _mediator, [FromBody] IntegrationEnquiryTcpRequest model) =>
                {
                    ExecResult<GeneralHuawiResponse> Result = await _mediator.Send(new IntegrationEnquiryCommand(new IntegrationEnquiryTcpRequest
                    {
                        Mss = model.Mss,
                        PrimaryIdentity = model.PrimaryIdentity,
                        PrimaryOffers = model.PrimaryOffers,
                        PrimaryOfferWhiteLists = model.PrimaryOfferWhiteLists
                    }));

                    return Results.Json(Result);
                })
                .WithName("IntegrationEnquery")
