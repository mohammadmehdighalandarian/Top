...
namespace TopinLite.CrmTransform.QueryBalance
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            app.MapPost("/QueryBalance", async (HttpContext httpContext, [FromServices] IMediator _mediator, [FromBody] QueryBalanceTcpRequest model) =>
                {
                    ExecResult<GeneralHuawiResponse> Result = await _mediator.Send(new QueryBalanceCommand(new QueryBalanceTcpRequest
                    {
                        Mss = model.Mss,
