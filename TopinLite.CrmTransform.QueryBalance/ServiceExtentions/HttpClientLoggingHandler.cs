...
namespace TopinLite.CrmTransform.QueryBalance.ServiceExtentions
{
    public class CustomLoggingFilter : IHttpMessageHandlerBuilderFilter
    {
        private readonly ILoggerFactory _loggerFactory;
        private readonly Infra.MsBroker.RabbitMQ.IRabbitMQBroker _rabbitMqBroker;
        ...
        public Action<HttpMessageHandlerBuilder> Configure(Action<HttpMessageHandlerBuilder> next)
        {
            ...
            return builder =>
            {
                ...
                builder.AdditionalHandlers.Insert(0, new CustomLoggingScopeHttpMessageHandler(outerLogger, _rabbitMqBroker));
            };
        }
    }
    ...
    public class CustomLoggingScopeHttpMessageHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            using (Log.BeginRequestPipelineScope(_logger, request))
            {
                ...
                HttpClientCallLogModel model = new()
                {
                    RequestBody = request.Content is null ? string.Empty : await request.Content.ReadAsStringAsync(cancellationToken),
                    RequestHeader = requestHeader,
                    ResponseBody = response.Content is null ? string.Empty : await response.Content.ReadAsStringAsync(cancellationToken),
                ...
                await _rabbitMqBroker.HttpCallLog(model, cancellationToken);
