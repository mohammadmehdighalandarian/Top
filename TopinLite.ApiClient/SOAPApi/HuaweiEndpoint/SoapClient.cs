...
namespace TopinLite.Infra.ApiClient.SOAPApi.HuaweiEndpoint
{
    public class SoapClient : ISoapClient
    {
        ...
        public async Task<TopinLite.Domain.HuaweiApiModel.CRMResponses.Recharge.EnvelopeRechargeResponse> RechargeByBroker(string PrimaryIdentity, decimal Amount, string Mss, string BrokerId, string RechargeChannelId, string TradeType, string BeId)
        {
            if (string.IsNullOrEmpty(Mss))
                Mss = "Topup/Topin" + Guid.NewGuid().ToString();
            using (HttpRequestMessage request = new(HttpMethod.Post, SoapServiceUrls.ArUrl))
