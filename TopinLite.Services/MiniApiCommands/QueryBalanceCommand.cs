...
{
    public class QueryBalanceCommand : IRpcHandler<QueryBalanceTcpRequest, GeneralHuawiResponse>
    {
        private readonly IEndpoint _IEndpoint;

        public QueryBalanceCommand(IEndpoint iEndpoint)
        {
            _IEndpoint = iEndpoint;
        }

        public ValueTask<RpcResult<GeneralHuawiResponse>> HandleAsync(RpcContext context, QueryBalanceTcpRequest request, CancellationToken cancellationToken)
        {
            try
            {
                GeneralHuawiResponse response = _IEndpoint.QueryBalance(new QueryBalanceTcpRequest
                {
                    Mss = request.Mss,
